...
class GameRepository
{
    ...
    {
        return Game::where('is_deleted',0)->orderBy('title')->get();
    }

    public function getGamesAdmin(): Collection
    {
    ...
    public function updateGame (int $game_id, string $title, int $publisher, array $category, int $release_year, string $description){
        ...
        $timestamp = date("Y-m-d H:i:s");
        if($alreadyExists == null || $alreadyExists->id == $game_id){
            ...
            return $message = "This game title already exists in the database";
        }
    }
